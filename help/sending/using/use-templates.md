...
**Uiteinde** - Adobe adviseert gebruikend een bestaand echt adres zoals de klantenzorg van uw merk. In dit geval, als een ontvanger een antwoord verzendt, zal de klantenzorg het kunnen behandelen.
...
Om de openingsfrequentie van uw leveringen te verhogen, raadt Adobe u aan een naam te gebruiken die gemakkelijk kan worden herkend door de ontvangers, zoals de naam van uw merk.
...
In de **Geavanceerde parameters** sectie van de eigenschappen van een malplaatje van SMS, **van** optie staat u toe om de naam van de SMS berichtafzender te personaliseren gebruikend een koord van karakters. Dit is de naam die wordt weergegeven als de naam van de afzender van het SMS-bericht op de mobiele telefoon van de ontvanger.

Als dit veld leeg is, wordt het bronnummer van de externe account gebruikt. Als er geen bronnummer wordt opgegeven, wordt de korte code gebruikt. Voor meer op dit, zie [&#x200B; configuratie van SMS &#x200B;](../../administration/using/configuring-sms-channel.md).

**Uiteinde** - controleer de wetgeving in uw land betreffende het wijzigen van het afzenderadres. Neem ook contact op met uw SMS-serviceprovider om te zien of deze deze functionaliteit aanbiedt.
